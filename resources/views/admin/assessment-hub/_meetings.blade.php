...
<div x-data="{ meetingTab: 'today', settingMeeting: false, upcomingMonth: 'all', archiveMonth: 'all' }"
    ...
    <div class="relative mb-5 inline-block w-full sm:w-56" x-data="{ open: false }"
    ...
    @foreach ([
        'today' => ['rows' => $meetingsToday, 'actions' => ['start'], 'months' => null],
        'upcoming' => ['rows' => $meetingsUpcoming, 'actions' => ['start', 'reschedule'], 'months' => $upcomingMonths],
        'archive' => ['rows' => $meetingsArchive, 'actions' => ['reschedule', 'delete'], 'months' => $archiveMonths],
    ] as $tabKey => $tabData)
    <div x-show="meetingTab === '{{ $tabKey }}'" @if ($tabKey !== 'today') x-cloak @endif>
        ...
        <div class="relative mb-4 inline-block w-full max-w-xs" x-data="{ open: false }"
            ...
            <button type="button" @click="open = !open"
                ...
                <span x-text="{{ $tabKey }}Month === 'all' ? 'All Months' : (@js($tabData['months'])[{{ $tabKey }}Month] ?? 'All Months')"></span>
                <svg class="h-4 w-4 shrink-0 text-gray-400 transition" :class="open && 'rotate-180'"
                    fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7" />
                </svg>
            ...
            <div x-show="open" x-cloak x-transition.origin.top
                ...
                style="display:none;">
                <button type="button" x-show="{{ $tabKey }}Month !== 'all'" @click="{{ $tabKey }}Month = 'all'; open = false"
                    class="block w-full px-3 py-2 text-left text-sm text-gray-700 transition hover:bg-gradient-to-r hover:from-[#6D0D23] hover:to-[#11386A] hover:text-white">
                    All Months
                </button>
                @foreach ($tabData['months'] as $monthKey => $monthLabel)
                <button type="button" x-show="{{ $tabKey }}Month !== '{{ $monthKey }}'" @click="{{ $tabKey }}Month = '{{ $monthKey }}'; open = false"
                    class="block w-full px-3 py-2 text-left text-sm text-gray-700 transition hover:bg-gradient-to-r hover:from-[#6D0D23] hover:to-[#11386A] hover:text-white">
                    {{ $monthLabel }}
                </button>
                @endforeach
            ...
            <div class="overflow-x-auto">
                <table class="w-full min-w-[680px] table-fixed text-sm">
                    ...
                    <tbody>
                        ...
                        <tr x-data="{ rescheduling: false, confirmingDelete: false }"
                            @if ($tabData['months'] !== null)
                            x-show="{{ $tabKey }}Month === 'all' || {{ $tabKey }}Month === '{{ $monthKeyOf($meeting) }}'"
                            @endif
                            class="border-b border-gray-100 last:border-0">
                            <td class="px-3 py-2 whitespace-nowrap text-left text-xs text-gray-600">{{ $meeting->time_range_label }}</td>
                        ...
                        <tr x-show="{{ $tabKey }}Month !== 'all' && ! (@js($tabData['rows']->map($monthKeyOf)->values())).includes({{ $tabKey }}Month)">
